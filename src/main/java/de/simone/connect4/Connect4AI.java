...
public class Connect4AI {
        ...
        for (int i = 0; i < width; i++) {
            if (root.children[i] != null) {
                if (maxIndex == -1 || root.children[i].visits > root.children[maxIndex].visits)
                    maxIndex = i;
                // System.out.printf("\nlocation%d: p1wins: %f/%d = %f", i,
                // root.children[i].player1Wins, root.children[i].visits,
                // root.children[i].player1Wins/root.children[i].visits);
            }
        }
        // System.out.println();
    ...
    private MCTSNode expand(MCTSNode selectedNode) {
        ...
        selectedNode.children[selectedIndex] = new MCTSNode(selectedNode,
                selectedNode.board.getNextState(selectedIndex));
    ...
    private class MCTSNode {
        ...
        // children[i] represents the next game state in which current player places
        // disc at location i
